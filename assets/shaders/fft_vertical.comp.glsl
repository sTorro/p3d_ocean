/*
 * fft_vertical.comp.glsl
 *
 * Author: storro
 * Date: 2026/02/11
 * Description: IFFT vertical pass compute shader (butterfly stages)
...
void main()
{
    ...
    vec4 a = imageLoad(u_input, ivec2(col, thread_idx));
    vec4 b = imageLoad(u_input, ivec2(col, thread_idx + thread_count));

    vec4 result0 = butterfly_operation(a.xy, b.xy, twiddle);
    vec4 result1 = butterfly_operation(a.zw, b.zw, twiddle);

    imageStore(u_output, ivec2(col, out_idx), vec4(result0.xy, result1.xy));
    imageStore(u_output, ivec2(col, out_idx + u_subseq_count), vec4(result0.zw, result1.zw));
}
